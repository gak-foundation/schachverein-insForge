...
function WaitlistTable({ applications }: { applications: Application[] }) {
  return (
            ...
            return (
              <tr key={app.id} className={`hover:bg-gray-50/50 transition-colors ${app.type === 'pilot' ? 'bg-primary/5' : ''}`}>
                ...
                <td className="px-6 py-4 text-right">
                  <WaitlistActions
                    id={app.id}
                    currentStatus={app.status as any}
                  />
                </td>
              </tr>
            );
...
export function WaitlistPageClient({ applications, statusCounts, currentStatus }: WaitlistPageClientProps) {
  const totalCount = (statusCounts.pending || 0) + (statusCounts.approved || 0) + (statusCounts.rejected || 0) + (statusCounts.waitlisted || 0);
